...
void LuaScriptInterface::registerFunctions()
{
  ...
  registerEnum(WEAPON_QUIVER)
	...
	registerMethod("Game", "removeItem", LuaScriptInterface::luaGameRemoveItem);
...
int LuaScriptInterface::luaGameRemoveItem(lua_State* L)
{
	// Game.removeItem(itemId[, count[, position]])
	uint16_t itemId;
	if (isNumber(L, 1)) {
		itemId = getNumber<uint16_t>(L, 1);
	} else {
	...
	for (int32_t i = 1; i <= itemCount; ++i) {
		...
		Item* item = Item::remove(itemId, stackCount);
		if (!item) {
			if (!hasTable) {
				lua_pushnil(L);
			}
