local config = {
	...
	storage = Storage.SoulWar.GoshnarsSpiteTime,
	requiredLevel = 400,
	leverId = 9825,
	...
	if item.itemid == config.leverId then
		...
		for participantPos = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(config.storage) > os.time() then
					...
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You or a member in your team have to wait ".. config.timeToFightAgain .."  hours to face Goshnar's Spite again!")
					return true
				end
		...
		for specPlayers = 1, #specs do
			...
			if spec:isPlayer() then
				player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "There's someone fighting with Goshnar's Spite.")
				return true
			end
			spec:remove()
		end
